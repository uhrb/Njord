...
{
    public class StandardClassBEquipmentReportMessageTests
    {
        private readonly ITestOutputHelper _helper;

        public StandardClassBEquipmentReportMessageTests(ITestOutputHelper helper)
        {
            _helper = helper;
        }
        ...
        [MemberData(
            ...
            "AisStreamMessages",
            nameof(AisStreamMessageType.StandardClassBPositionReport),
        ...
        public void ConvertedWithoutExceptions(ulong id, string message, bool valid)
        {
            ...
            Assert.Equal(AisStreamMessageType.StandardClassBPositionReport, result.MessageType);
            Assert.NotNull(result.Metadata);
            Assert.NotNull(result.Message);
            var typedMessage = result.Message as IStandardClassBEquipmentReportMessage;
        ...
        public void ConvertedBackAndForce(ulong id, string message, bool valid)
        {
            ...
            Assert.Equal(AisStreamMessageType.StandardClassBPositionReport, result2.MessageType);

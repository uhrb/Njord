...
{
    public class ExtendedClassBEquipmentPositionReportMessageTests 
    {
        private readonly ITestOutputHelper _helper;

        public ExtendedClassBEquipmentPositionReportMessageTests(ITestOutputHelper helper) 
        {
            _helper = helper;
        }
        ...
        [MemberData(
            ...
            "AisStreamMessages",
            nameof(AisStreamMessageType.ExtendedClassBPositionReport),
        ...
        public void ConvertedWithoutExceptions(ulong id, string message, bool valid)
        {
            ...
            Assert.Equal(AisStreamMessageType.ExtendedClassBPositionReport, result.MessageType);
            Assert.NotNull(result.Metadata);
            Assert.NotNull(result.Message);
            var typedMessage = result.Message as IExtendedClassBEquipmentPositionReportMessage;
        ...
        public void ConvertedBackAndForce(ulong id, string message, bool valid)
        {
            ...
            Assert.Equal(AisStreamMessageType.ExtendedClassBPositionReport, result2.MessageType);

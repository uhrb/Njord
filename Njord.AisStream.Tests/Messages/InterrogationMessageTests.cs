...
{
    public class InterrogationMessageTests
    {
        private readonly ITestOutputHelper _helper;

        public InterrogationMessageTests(ITestOutputHelper helper)
        {
            _helper = helper;
        }
        ...
        [MemberData(
            ...
            "AisStreamMessages",
            nameof(AisStreamMessageType.Interrogation),
            MemberType = typeof(MessagesDataClass)
        )]
        public void ConvertedWithExceptions(ulong id, string message, bool valid)
        {
            ...
            Assert.Equal(AisStreamMessageType.Interrogation, result.MessageType);
            Assert.NotNull(result.Metadata);
            Assert.NotNull(result.Message);
            var typedMessage = result.Message as IInterrogationMessage;
        ...
        public void ConvertedBackAndForce(ulong id, string message, bool valid)
        {
            ...
            Assert.Equal(AisStreamMessageType.Interrogation, result2.MessageType);
